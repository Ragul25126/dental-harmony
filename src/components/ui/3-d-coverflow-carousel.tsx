...
export function CoverFlowCarousel({
  ...
  const prevSlide = useCallback(() => {
    setInternalIndex((prev) => (prev - 1 + total) % total);
  }, [total]);

  const goToSlide = (idx: number) => {
    setInternalIndex(idx % total);
  };
  ...
  useEffect(() => {
    if (!autoplay || scrollDriven || externalIndex !== undefined || isHovered || total <= 1) return;
    const interval = setInterval(nextSlide, autoplayDelay);
    return () => clearInterval(interval);
  }, [autoplay, autoplayDelay, externalIndex, isHovered, nextSlide, scrollDriven, total]);
            ...
            return (
              <div
                ...
                style={{
                  ...
                  border: "1px solid rgba(25, 51, 49, 0.15)",
                  transform,
                  opacity,
                  zIndex,
                  filter,
